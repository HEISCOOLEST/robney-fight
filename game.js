...
document.getElementById("joinRoom").addEventListener("click",async()=>{
  ...
  try{
    ...
    await updateDoc(ref,{players,status:"ready","p2.x":Math.max(220,window.innerWidth-180),"p2.y":0,"p2.health":100,"p2.momentum":0,"p2.action":"idle"});
    openGame();
  }catch(e){console.error(e);status.textContent=e.message}
});
...
document.getElementById("copyRoom").addEventListener("click",async()=>{
  ...
  }catch(e){showMessage("ROOM: "+roomCode)}
});
...
function startRoomListener(){
  onSnapshot(doc(db,"rooms",roomCode),snap=>{
    ...
    if(roomData.status==="ready" && (roomData.players||[]).length>=2)gameStarted=true;
