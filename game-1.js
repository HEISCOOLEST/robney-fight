...
async function enterFullscreen(){const el=document.documentElement;try{if(el.requestFullscreen)await el.requestFullscreen();else if(el.webkitRequestFullscreen)el.webkitRequestFullscreen()}catch{}}
async function exitFullscreen(){try{if(document.exitFullscreen)await document.exitFullscreen();else if(document.webkitExitFullscreen)document.webkitExitFullscreen()}catch{}}
async function toggleFullscreen(){if(fsEl())await exitFullscreen();else await enterFullscreen()}
function syncFsBtn(){fsBtn?.classList.toggle("is-full",!!fsEl())}
function onFsChange(){syncFsBtn();resizeCanvas();checkOrientation()}
if(fsBtn){if(fsSupported())fsBtn.classList.add("supported");fsBtn.addEventListener("click",toggleFullscreen);document.addEventListener("fullscreenchange",onFsChange);document.addEventListener("webkitfullscreenchange",onFsChange)}
